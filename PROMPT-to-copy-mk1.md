...
## Public API Contract (must match exactly — do not rename or restructure)

To allow this package to be verified with an identical external smoke-test
script regardless of which coding agent built it, the public entry point
must match this exact shape:

- Namespace: `HumanNotFound\MysqlSshTunnel`
- Class `TunnelConfig` (immutable value object):
  - Static factory: `TunnelConfig::fromArray(array $config): self`
    — accepts the config array shape described earlier in this document
    (`local_port`, `server`, `ssh_user`, `ssh_port`, `remote_port`,
    `ssh_binary_path`, `ssh_key_path`, `environments`, `current_environment`).
    Throws `TunnelException` for config validation errors only. Additional
    optional keys beyond these (e.g. a PSR-3 `logger`, a `connect_timeout`,
    `strict_host_key_checking`) are permitted at your discretion but not
    required.
- Class `TunnelResult` (immutable/readonly outcome object):
  - Public readonly properties: `bool $active`, `int $localPort`,
    `string $host`.
  - When `$active` is true, connect to `127.0.0.1:$localPort`. When false,
    the library has fallen back to a direct connection: `$localPort` carries
    the *remote* port and `$host` carries the remote server hostname —
    `$host`/`$localPort` can be fed into a DSN without branching, but code
    that only sees the global constants must still branch on
    `MYSQL_SSH_TUNNEL_ACTIVE` to pick the right host.
- Class `TunnelManager`:
  - Static one-call bootstrap: `TunnelManager::boot(array $config): TunnelResult`
    — builds the `TunnelConfig`, ensures the tunnel (detect/reuse/start/
    fallback), defines the global constants, and returns the `TunnelResult`.
    This is the single entry point framework adapters and standalone scripts
    should use.
  - Constructor takes a `TunnelConfig` instance directly, for callers who
    need to separate config-building from tunnel-establishment.
  - Instance method `ensure(): TunnelResult` — the detect/reuse/start/
    fallback logic without defining constants.
  - Instance method `defineConstants(TunnelResult $result): void` — defines
    `MYSQL_SSH_TUNNEL_LOCAL_PORT` and `MYSQL_SSH_TUNNEL_ACTIVE`, guarded by
    `defined()` so multiple bootstrap paths in one request don't collide.
  - No public manual teardown method is required — the shutdown behavior
    described earlier (register a `register_shutdown_function()` only for
    random-port tunnels) is sufficient on its own and needs no separate
    testing hook.

Usage must look like this in every framework adapter and the standalone
example:
```php
...
// $result->active / $result->localPort / $result->host carry the same
// information directly, for callers who prefer not to read constants.
```
Internal implementation details (private methods, additional helper classes,
constructor injection for a logger, process-runner seams for testing, etc.)
are entirely up to you — only this public surface is fixed.

```
php-mysql-over-ssh-tunnel/
  composer.json
  src/
    TunnelConfig.php       // immutable config value object + validation
    TunnelManager.php       // core: start/detect/reuse/shutdown logic
    TunnelException.php
  examples/
    ...
    standalone.php
  tests/
    TunnelManagerTest.php   // PHPUnit, with proc_open mocked/faked
  README.md
  AGENT.md
  composer.json
```
...
- [ ] `composer.json`
- [ ] `src/TunnelConfig.php`, `src/TunnelManager.php`, `src/TunnelException.php`
